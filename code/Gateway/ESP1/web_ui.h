...
<head>
  ...
  <style>
    ...
    *{margin:0;padding:0;box-sizing:border-box}
    body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:var(--bg-primary);color:var(--text-primary);min-height:100vh;overflow-x:hidden;line-height:1.5;font-size:calc(15px * var(--ui-font-scale));transition:background .3s,color .3s}
    a{color:inherit}
    ...
    .main{flex:1;min-width:0;margin-left:240px;padding:20px;min-height:100vh}
    ...
    .traffic-toolbar{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:10px;margin-bottom:14px}.traffic-list{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px;align-items:start}.traffic-card{padding:16px;border-radius:8px;background:var(--bg-tertiary);border:1px solid var(--border-primary);min-width:0}.traffic-head{display:flex;justify-content:space-between;gap:12px;flex-wrap:wrap;margin-bottom:10px}.traffic-badges{display:flex;gap:8px;flex-wrap:wrap;align-items:center}.traffic-meta{color:var(--text-muted);font-size:10px;font-family:monospace}.traffic-pre{margin:0;padding:14px;border-radius:8px;background:rgba(15,23,42,.22);overflow-x:auto;white-space:pre-wrap;word-break:break-word}.traffic-terminal-wrap{position:relative}.traffic-terminal{padding:12px 14px;border-radius:10px;border:1px solid var(--border-primary);background:#05080d;color:#8bffb0;font-family:monospace;font-size:calc(11px * var(--ui-font-scale));line-height:1.45;max-height:38vh;min-height:120px;overflow-y:auto;overflow-x:auto}.traffic-jump-btn{position:absolute;bottom:14px;right:24px;padding:6px 14px;border-radius:20px;border:1px solid var(--accent-primary);background:var(--accent-primary);color:#fff;font-size:11px;font-weight:600;cursor:pointer;box-shadow:0 4px 12px rgba(0,0,0,.35)}.traffic-jump-btn:hover{filter:brightness(1.1)}.traffic-terminal-line{white-space:nowrap;margin-bottom:2px}.traffic-terminal-line:last-child{margin-bottom:0}.empty-state{padding:24px;border-radius:16px;border:1px dashed var(--border-primary);color:var(--text-muted);text-align:center}.node-grid{display:flex;flex-wrap:wrap;gap:12px;align-items:flex-start;justify-content:flex-start;align-content:flex-start;max-height:270px;overflow-y:auto;padding-right:4px}.node-card{background:var(--bg-tertiary);border:1px solid var(--border-primary);border-radius:12px;padding:14px;display:inline-grid;gap:10px;flex:0 0 auto;width:fit-content;min-width:0;max-width:100%}.node-card-header{display:flex;align-items:flex-start;justify-content:flex-start;gap:10px}.node-name{font-size:calc(14px * var(--ui-font-scale));font-weight:700;color:var(--text-primary);word-break:break-word}.node-meta{font-size:calc(11px * var(--ui-font-scale));color:var(--text-muted)}.node-badges{display:flex;flex-wrap:wrap;gap:6px}.node-card.status-valid{border-color:rgba(63,185,80,.34)}.node-card.status-wrong_key,.node-card.status-invalid_json,.node-card.status-missing_id,.node-card.status-crc_error{border-color:rgba(248,81,73,.24)}
    .badge.source-lora{background:var(--accent-glow);color:var(--accent-primary)}.badge.source-espnow{background:rgba(59,130,246,.14);color:#58a6ff}.badge.status-valid{background:var(--success-glow);color:var(--success)}.badge.status-invalid_json,.badge.status-wrong_key,.badge.status-missing_id,.badge.status-crc_error{background:var(--danger-glow);color:var(--danger)}.badge.status-mqtt_offline{background:var(--warning-glow);color:var(--warning)}.badge.node-name{background:rgba(168,85,247,.14);color:#c084fc}.traffic-card.source-lora{box-shadow:inset 0 0 0 1px rgba(240,136,62,.08)}.traffic-card.source-espnow{box-shadow:inset 0 0 0 1px rgba(88,166,255,.08)}.traffic-card.status-valid{border-color:rgba(63,185,80,.35)}.traffic-card.status-invalid_json,.traffic-card.status-wrong_key,.traffic-card.status-missing_id,.traffic-card.status-crc_error{border-color:rgba(248,81,73,.35)}.traffic-card.status-mqtt_offline{border-color:rgba(210,153,34,.35)}
...
<body>
  ...
  <div class="app">
    ...
    <main class="main">
      ...
      <section class="page" id="page-monitoring">
        ...
        <div class="card"><div class="card-header"><h3 class="card-title"><svg viewBox="0 0 24 24"><path d="M3 12h4l3 8 4-16 3 8h4"/></svg>Prettified Payloads</h3><span class="badge warning">RAM buffer</span></div><div class="traffic-toolbar"><select class="form-select" id="trafficSourceFilter"><option value="all">All sources</option><option value="LoRa">LoRa</option><option value="ESP-NOW">ESP-NOW</option></select><select class="form-select" id="trafficStatusFilter"><option value="all">All statuses</option><option value="valid">Valid</option><option value="wrong_key">Wrong key</option><option value="missing_id">Missing ID</option><option value="invalid_json">Invalid JSON</option><option value="crc_error">CRC error</option><option value="mqtt_offline">MQTT offline</option></select></div><div class="traffic-list" id="trafficList"></div></div><div class="card traffic-terminal-wrap"><div class="card-header"><h3 class="card-title"><svg viewBox="0 0 24 24"><path d="M8 9l3 3-3 3M13 15h3"/></svg>Raw View</h3><span class="badge warning">Live stream</span></div><div class="traffic-terminal" id="trafficTerminal"></div><button type="button" class="traffic-jump-btn" id="trafficJumpBtn" hidden>New output &#8595;</button></div>
      </section>
      ...
      <section class="page" id="page-settings">
        ...
        <form id="settingsForm">
          <div class="grid-2">
            ...
            <div class="card"><div class="card-header"><h3 class="card-title"><svg viewBox="0 0 24 24"><path d="M12 2 2 7l10 5 10-5-10-5Z"/><path d="M2 17l10 5 10-5M2 12l10 5 10-5"/></svg>MQTT</h3></div><div class="grid-2"><div class="form-group"><label class="form-label" for="mqttServer">Server</label><input class="form-input" id="mqttServer" maxlength="64" placeholder="hostname or IP"></div><div class="form-group"><label class="form-label" for="mqttPort">Port</label><input class="form-input" id="mqttPort" type="number" min="1" max="65535"></div></div><div class="grid-2"><div class="form-group"><label class="form-label" for="mqttUsername">Username</label><input class="form-input" id="mqttUsername" maxlength="64"></div><div class="form-group"><label class="form-label" for="mqttPassword">Password</label><input class="form-input" id="mqttPassword" maxlength="64"></div></div><div class="toggle-group"><div class="toggle-info"><span class="toggle-title">Use TLS</span><span class="toggle-desc">Server-only TLS (no client cert) validated against Let's Encrypt's ISRG Root X1. Use port 8883.</span></div><div class="toggle-btn" id="mqttUseTls" data-toggle="false" onclick="toggleSettingButton('mqttUseTls')"></div></div></div>
          </div>
          <div class="card"><div class="card-header"><h3 class="card-title"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>Time (SNTP)</h3></div><div class="toggle-group"><div class="toggle-info"><span class="toggle-title">Enable SNTP</span><span class="toggle-desc">Off by default. When off, the UI shows uptime instead of real timestamps for received events.</span></div><div class="toggle-btn" id="ntpEnabled" data-toggle="false" onclick="toggleSettingButton('ntpEnabled')"></div></div><div class="grid-2"><div class="form-group"><label class="form-label" for="ntpServer">NTP Server</label><input class="form-input" id="ntpServer" maxlength="64"></div><div class="form-group"><label class="form-label" for="utcOffsetMinutes">UTC Offset (minutes)</label><input class="form-input" id="utcOffsetMinutes" type="number" min="-720" max="840"></div></div><div class="form-hint">Add DST manually if needed.</div></div>
          <div class="grid-2">
  ...
  <script>
    ...
    const trafficJumpBtn=document.getElementById('trafficJumpBtn');
    ...
    function renderTraffic(items){
      ...
      const wasNearBottom=trafficTerminal.scrollHeight-trafficTerminal.scrollTop-trafficTerminal.clientHeight<=40;
      ...
      if(wasNearBottom){
        ...
        trafficJumpBtn.hidden=true;
      }else{
        trafficJumpBtn.hidden=false;
      }
    }

    trafficJumpBtn.addEventListener('click',()=>{
      trafficTerminal.scrollTop=trafficTerminal.scrollHeight;
      trafficJumpBtn.hidden=true;
    });
    ...
    function fillForm(settings){
      ...
      setToggleButton('mqttUseTls',!!settings.mqttUseTls);
      setToggleButton('ntpEnabled',!!settings.ntpEnabled);
      document.getElementById('ntpServer').value=settings.ntpServer||'';
      document.getElementById('utcOffsetMinutes').value=settings.utcOffsetMinutes ?? 0;
